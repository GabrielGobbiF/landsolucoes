...
@section('content')
            ...
            <form id="form-drivers" role="form" class="needs-validation" novalidate autocomplete="off" action="{{ route('drivers.update', $driver->id) }}"
                ...
                <div class="row">
                    <div class="col-md-12 d-flex justify-content-between">
                        ...
                        <button type="button" class="btn btn-outline-primary" data-toggle="modal" data-target="#driverFile">
                            Novo arquivo
                        </button>
                    </div>
                </div>
                                        ...
                                        <div class="dropdown-file" style="position: absolute;right: 4px;top: 8px;">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <form id="form-delete-" role="form" class="needs-validation"
                                                      ...
                                                      action="{{ route('vehicles.drivers.files.destroy', [$driver->id, $file->token]) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
    ...
    <div id="driverFile" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modalTitleId" aria-hidden="true">
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        let resumable = new Resumable({
            target: '{{ route('vehicles.drivers.files', $driver->id) }}',
            query: {
                _token: '{{ csrf_token() }}'
            }, // CSRF token
