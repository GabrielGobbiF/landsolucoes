            ...
            <div class="filemgr-content-body ps ps--active-y">
                <div class="pd-20 pd-lg-25 pd-xl-30">
                    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
                        ...
                        @else
                            <div>
                                ...
                                <label
                                    class="d-block tx-medium tx-10 tx-uppercase tx-sans tx-spacing-1 tx-color-03 mg-b-15">Recentes
                                    Adicionados</label>
                            </div>
                        @endif
                        <div class="tollbar btn-toolbar mb-2 mb-md-0 float-right">
                            <form id='form-search-doc' role='form' class='needs-validation'
                                action='{{ route('arquivos.index') }}' method='GET'>
                    ...
                    @if (count($directorys) > 0)
                        ...
                        <label
                            class="d-block tx-medium tx-10 tx-uppercase tx-sans tx-spacing-1 tx-color-03 mg-b-15">Pastas</label>
                        <div class="row">
                                    ...
                                    <div class="media media-folder">
                                        ...
                                        <div class="media-body">
                                            <h6><a href="{{ route('folder.show', $directory->uuid) }}"
                                                    class="link-02">{{ ucfirst(mb_strtolower(mb_strimwidth($directory->name, 0, 35, '...'), 'utf-8')) }}</a>
                                            </h6>
                                            <span>{{ $directory->documentos_count }} files</span>
                                        </div>
                                        <div class="dropdown-file">
                                            <a href="{{ route('folder.show', $directory->uuid) }}" class="dropdown-link"
                                                data-toggle="dropdown"><i data-feather="more-vertical"></i></a>
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <form id="form-delete-{{ $directory->id }}" role="form"
                                                    ...
                                                    <button type="submit" id="modal-confirm" data-btn-text="Deletando"
                                                        class="dropdown-item delete"><i
                                                            data-feather="trash"></i>Deletar</button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    @if (count($obrasPastas) > 0)
                        <hr class="mg-y-40 bd-0">
                        <label
                            class="d-block tx-medium tx-10 tx-uppercase tx-sans tx-spacing-1 tx-color-03 mg-b-15">Obras</label>
                        <div class="row">
                                    ...
                                    <div class="media media-folder">
                                        ...
                                        <div class="media-body">
                                            <h6><a href="{{ route('folder.show', $obrasPasta->uuid) }}"
                                                    class="link-02">{{ ucfirst(mb_strtolower(mb_strimwidth($obrasPasta->name, 0, 35, '...'), 'utf-8')) }}</a>
                                            </h6>
                                            <span>{{ $obrasPasta->documentos_count }} files</span>
                                        </div>
                                        <div class="dropdown-file">
                                            <a href="{{ route('folder.show', $obrasPasta->uuid) }}" class="dropdown-link"
                                                data-toggle="dropdown"><i data-feather="more-vertical"></i></a>
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <form id="form-delete-{{ $obrasPasta->id }}" role="form"
                                                    class="needs-validation"
                                                    action="{{ route('pastas.destroy', $obrasPasta->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" id="modal-confirm" data-btn-text="Deletando"
                                                        class="dropdown-item delete"><i
