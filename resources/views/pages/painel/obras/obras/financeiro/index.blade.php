...
@section('content-max-fluid')
    ...
    <section class="invoice">
        <div class="invoice-grid  justify-content-center">
            ...
            <div class="d-flex justify-content-center">
                <div class="box box-solid box-invoice">
                            ...
                            <table class='table table-hover table-condensed'>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($dadosFinanceirosCollection as $dadosEtapa)
                                        ...
                                        @php
                                            if ($dadosEtapa['id'] == '1569') {
                                                #dd([$dadosEtapa['a_receber']]);
                                            }

                                            $recebido = bccomp((string) $dadosEtapa['recebido'], (string) $dadosEtapa['valor_total'], 2) === 0;

                                        @endphp

                                        <tr class="{{ $recebido ? 'cross-line' : '' }}">

                                            {{-- Etapa de faturamento --}}
                                            <th>
                                                <div class="d-grid">
                                                    <span class="">{{ $dadosEtapa['nome_etapa'] }}</span>
                                                    <span style="font-size: small;" class="text-muted text-sm">{{ $dadosEtapa['nome'] }}</span>
                                                    <span class="text-{{ $dadosEtapa['vencidas']['quantidade'] > 0 ? 'danger' : '' }}">
                                                        Vencidas: {{ $dadosEtapa['vencidas']['quantidade'] }}
                                                    </span>
                                                </div>
                                            </th>
                                            ...
                                            <th>
                                                @if (bccomp((string) $dadosEtapa['total_faturado'], '0', 2) !== 0 &&
                                                        bccomp((string) $dadosEtapa['total_faturado'], (string) $dadosEtapa['valor_total'], 2) === 0)
